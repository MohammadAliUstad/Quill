    ...
    AndroidView(
        ...
        factory = { ctx ->
            ImageView(ctx).apply {
                ...
                post {
                    // setImageState triggers the AnimatedStateListDrawable's transition
                    // animation properly, unlike setting isActivated directly
                    val activatedState = intArrayOf(android.R.attr.state_activated)
                    val emptyState = intArrayOf()
                    drawable?.setState(if (isLiked) activatedState else emptyState)
                }
            }
        },
        update = { imageView ->
