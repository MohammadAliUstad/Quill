...
// Plain data holder returned by fetchQuota
data class QuotaData(
    val queriesUsed: Int,
    val queriesLimit: Int,
    val resetAt: Timestamp?
) {
    // True if the reset time has passed — caller should reset before checking usage
    val isExpired: Boolean
        get() = resetAt != null && resetAt.toDate().before(java.util.Date())

    // True if the user still has queries available
    val hasQuota: Boolean
        get() = queriesUsed < queriesLimit

    // Remaining queries for display in the UI
    val remaining: Int
